describe("API Auth Token", function () {
    ...
    before(async function({ supertest }) {
        // Generate the Auth Token only once
        await supertest
    ...
    for (let i = 0; i < 3; i++) {
        it(`Post API Test After Login - Iteration ${i + 1}`, async function({ supertest }) {
            await supertest
                ...
                .catch(function (error) {
                    console.error(`Error in Post API Test After Login - Iteration ${i + 1}:`, error);
                });
        });

        it(`Get API Test After Login - Iteration ${i + 1}`, async function({ supertest }) {
            await supertest
                .request("https://ihgce1ua.hospitality-api.eu-frankfurt-1.ocs.oc-test.com")
                ...
                .then(function(response) {
                    ...
                    console.log(`Status: Reservation created successfully in OHIP - Iteration ${i + 1}`);
                    console.log(`Reservation ID: ${reservationId}`);
                    console.log('Confirmation ID:', confirmationId);
                    console.log('External Reference ID:', externalReferenceId);
                })
                .catch(function (error) {
                    console.error(`Error in Get API Test After Login - Iteration ${i + 1}:`, error);
                });
        });
    }
});
