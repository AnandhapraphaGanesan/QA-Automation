...
/* Create an Config.json file in common directory and mention filepath for input and output files
look like the below code 
{
    "filepath": {
      "jsonfilePath" : "./tests/step_definitions/api/RESVFLOWRAJA/Reservation_flowtd.json" ,
      "xlsfilepath" : "./tests/step_definitions/api/RESVFLOWRAJA/reservflows.xlsx"     
    }
}*/
...
it('GET Reservation GRS', async function ({ supertest }) {
    ...
    it('POST Cancel Reservation', async function ({ supertest }) {  
        await supertest
          ...
          .then(function (response) {
            ...
            const reservation = response.body.reservations[0]; // Select only the first reservation

      const reservationIdObj = reservation.reservationIdList.find(item => item.type === 'Reservation');
      const confirmationIdObj = reservation.reservationIdList.find(item => item.type === 'Confirmation');
      const cancellationIdObj = reservation.reservationIdList.find(item => item.type === 'Cancellation');

      const reservationId = reservationIdObj ? reservationIdObj.id : 'Not Found';
      const confirmationId = confirmationIdObj ? confirmationIdObj.id : 'Not Found';
      const cancellationId = cancellationIdObj ? cancellationIdObj.id : 'Not Found';
      const cancellationStatus = reservation.reservationStatus;
 ...
 it('GET Reservation cancel OHIP', async function ({ supertest }) {
    await supertest
        .request(reservation.request)
        .get(reservation.Getendpath + reservationId)
        .set('Content-Type', reservation['Content-Type1'])
...
it('GET Reservation cancel GRS', async function ({ supertest }) {
    await supertest
        .request(reservation.request1)
        .get(reservation.Getendpath1 + externalReferenceId)
        .query({
        ...
        .then(function (response) {
            ...
            const pmsConfirmationNumberEntry = reservation.reservationIds.confirmationNumbers.find(entry => entry.pmsConfirmationNumber);
            pmsConfirmationNumber = pmsConfirmationNumberEntry ? pmsConfirmationNumberEntry.pmsConfirmationNumber : 'Not found';
            const cancellationId1 = responseBody.hotelReservation.reservationIds.cancellationNumber;
            let cancellationId2 = cancellationId1;
            const cancellationStatus1 = responseBody.hotelReservation.reservationStatus;
            let cancellationStatus2 = cancellationStatus1;

            console.log("GET Cancellation ID:", cancellationId1);
            console.log("GET Cancellation Status:", cancellationStatus1); 
            console.log("Status: Reservation created Successfully in GRS");
            console.log("IHG Confirmation Number:", ihgConfirmationNumber);
            console.log("External Confirmation Number:", externalConfirmationNumber);
            console.log("PMS Confirmation Number:", pmsConfirmationNumber);
            excelData.push([reservationId, confirmationId,externalReferenceId,ihgConfirmationNumber,externalConfirmationNumber,pmsConfirmationNumber,cancellationId2,cancellationStatus2]);
        })
});
...
after(function () {
    ...
    const sheetName = 'flow1';  // Change this for each program (e.g., 'Program1', 'Program2', etc.)
 
    // Use an object to map sheet names to their corresponding headers
    const headersMap = {
        'flow1': ['Reservation ID', 'Confirmation ID', 'External Reference ID', 'IHG Confirmation Number', 'External Confirmation Number', 'PMS Confirmation Number', 'CancellationId1', 'CancellationStatus1'],
